...
class Graph(object):
    ...
    def has_cycle(self):
        for i in range(len(self.Vertices)):
            # input random vertex each time
            lst = (self.dfs(random.randint(0, len(self.Vertices)-1)))

            # if function detects a cycle within for loop
            if(lst == True):
                return True
            
        # if function runs through for loop without detecting cycle
        return lst
    ...
    def toposort(self):
        ...
        # initiaize topoList and count of vertices
        topoList = []
        visited = 0

        # call helper function until all vertices are visited
        while(visited < len(self.Vertices)):
            # input random vertex each time
            lst = (self.toposort_helper(random.randint(0, len(self.Vertices)-1)))

            # check if vertex has already been added to topoList
            for i in lst:
                if(i not in topoList):
                    topoList.append(i)
                    visited += 1        # add to visited for each appended node
    ...
    def toposort_helper(self,random_vertex):
        # each time we backtrack, add to topolist
        sub_topoList = []
        theStack = Stack()
        ...
        while(not theStack.is_empty()):
            ...
            if (u == -1):
                ...
                # only append to topoList if not already added
                if(self.Vertices[u].label not in sub_topoList):
                    sub_topoList.append(self.Vertices[u].label)

            else:                                                  # if v does have adjacent vertexes
                (self.Vertices[u]).visited = True
                theStack.push(u)
